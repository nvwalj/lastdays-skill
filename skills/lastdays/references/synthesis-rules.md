...
- **Engagement honesty.** Quote real numbers only from engine items that carry them (points/upvotes/comments/volume). Two kinds of items have NO engagement and must never be given a fabricated count: (a) Web/X/Chinese items gathered via WebSearch — label them "web-sourced"; (b) **degraded engine items**, flagged `⚠ degraded:<tier>` in the evidence (Google News articles, arXiv papers — RSS/Atom feeds with no upvotes/citations). Cite degraded items by their outlet/author (the publisher, the paper authors), rank them below comparable engagement-bearing items, and treat arXiv as legitimately empty for non-research topics. Never fabricate a like/upvote count for any of these.
- **Lead with relevance, not engagement.** Each engine item line shows `rel=` (0–0.9). A high-engagement item with low `rel` is off-topic noise the search loosely recalled — do NOT feature it just because it's popular. Build the brief from high-`rel` items; mention a low-`rel` item only if it genuinely informs the topic.
- **Honor the engine's weak-signal flags.** A source header may carry `⚠ no strongly-relevant results in-window` (every item is a weak/floor match) — treat that source as "nothing on-topic this window" rather than forcing its top item in. If the engine prints a global `NOTE: no strongly-relevant engine results …`, the whole pool is weak for this topic: lean on the WebSearch layers, lower confidence explicitly, and do NOT present floored items as established findings.
