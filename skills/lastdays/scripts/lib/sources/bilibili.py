...
Sorted by publish date so the window filter keeps the freshest hits.
    ...
    for v in results or []:
        ...
        items.append(
            Item(
                ...
                snippet=strip_html(v.get("description", ""))[:240],
                relevance=0.6,
                item_id=f"bv{bvid}",
...
def fetch(query: str, window: Window, *, env: dict, depth: str = "default") -> list[Item]:
    ...
    params = {
        "search_type": "video",
        "keyword": query,
        "order": "pubdate",     # newest first, so the window filter keeps fresh hits
        "page": "1",
        "page_size": str(DEPTH.get(depth, 20)),
        "platform": "pc",
    ...
    code = resp.get("code")
    if code != 0:
        # surface B站's own error (e.g. -412 风控) so the engine records it
