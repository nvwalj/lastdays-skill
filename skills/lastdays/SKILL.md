...
description: >-
  ...
  and Polymarket with real engagement (upvotes, points, comments, volume) and a
  strict date window; you supplement the open web and X via WebSearch and
...
The engine prints an `EVIDENCE FOR SYNTHESIS` block (ranked Reddit / Hacker News / GitHub / Polymarket items with real engagement, strictly inside the window) and a `WEB LAYERS TO FILL` list. Read the evidence; **do not** paste it back verbatim. Run `--diagnose` once if you want to see which sources are live vs stub.
...
- `--sources reddit,hackernews,github,polymarket` — pick a subset (aliases: `hn`, `gh`, `pm`, `r`).
